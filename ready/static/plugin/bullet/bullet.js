...
$.fn.bullet = function (options) {
    ...
    if (flg) {
        var css = '.bullet {overflow-x:hidden; overflow-y:scroll\9;margain:0 auto;}.bullet .bulletBox {position: absolute;}.bullet .bulletContent { position: relative; z-index: 1000; border-radius: 30px; background: rgba(0,0,0,.5); padding: 2px 8px; -webkit-font-smoothing: antialiased; display: inline-block; margin-left: 60px; float: left; color: #fff;white-space:nowrap; }';
    ...
    $(document).ready(function () {
        $(".bullet").find(".bulletBox").each(function () {
            ...
            the.css("width", contentWidth).hide();
            var number = 0;
            var boxWidth = params.boxwidth;
            setInterval(function () {
                the.show();
                if (number >= contentWidth) { number = 0; boxWidth = params.boxwidth; }
                if (boxWidth > 0) {
                    the.css("-moz-transform", "translate3d(" + (boxWidth = (boxWidth - params.speed)) + "px,0px,0px)");
                    the.css("-webkit-transform", "translate3d(" + (boxWidth = (boxWidth - params.speed)) + "px,0px,0px)");
                    the.css("-ms-transform", "translate(" + (boxWidth = (boxWidth - params.speed)) + "px,0px)");//IE9仅适用于2D转换
                    the.css("-o-transform", "translate(" + (boxWidth = (boxWidth - params.speed)) + "px,0px)");//Opera仅适用于2D转换
                    the.css("transform", "translate3d(" + (boxWidth = (boxWidth - params.speed)) + "px,0px,0px)");
